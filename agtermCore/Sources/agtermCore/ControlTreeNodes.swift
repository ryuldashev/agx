...
// The read-back tree `tree` / `window list` project: one node per surface, HUD, session, workspace, window.
...
public struct ControlSessionNode: Codable, Sendable, Equatable {
    ...
    /// A private session (ADR 0005): not persisted, its agent's files erased on close; nil/omitted otherwise.
