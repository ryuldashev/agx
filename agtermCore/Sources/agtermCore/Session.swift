...
public final class Session: Identifiable {
    ...
    /// A private session (ADR 0005) is never persisted: no snapshot, no recent-closed entry, no durable
    ...
    /// Clears the HUD's state together, so `hudActive` and the read-back cannot outlive the slot.
    public func discardHud() {
        hudSpec = nil
