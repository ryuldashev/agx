...
extension AppStore {
    ...
    public func snapshot() -> Snapshot {
        // a private session (ADR 0003) is never written: not as a row, not as the selection, not in recency.
    ...
    func sessionSnapshot(_ session: Session) -> SessionSnapshot {
        SessionSnapshot(id: session.id, customName: session.customName, cwd: session.currentCwd ?? session.initialCwd,
                        ...
                        splitRestoreCommand: session.splitRestoreCommand, title: session.oscTitle)
    }
