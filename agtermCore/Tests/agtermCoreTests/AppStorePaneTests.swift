...
struct AppStorePaneTests {
    ...
    @Test func openHudOccupiesTheSlotAndMarksItAHud() {
        ...
        #expect(store.openHud(session.id, spec: spec, size: HudPanelSize(widthPercent: 30, heightPercent: 9)) == true)
        #expect(session.overlayActive == true)
        ...
        store.openHud(session.id, spec: spec, size: HudPanelSize(widthPercent: 400, heightPercent: 9))
        #expect(session.overlaySizePercent == HudLayout.maxSizePercent)
        store.closeHud(session.id)
        store.openHud(session.id, spec: spec, size: HudPanelSize(widthPercent: 100, heightPercent: 9))
        #expect(session.overlaySizePercent == HudLayout.maxSizePercent)
        store.closeHud(session.id)
        store.openHud(session.id, spec: spec, size: HudPanelSize(widthPercent: 1, heightPercent: 9))
    ...
    @Test func updateHudRewritesInPlaceWithoutRespawning() {
        ...
        let ws = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: ws.id, cwd: "/a")!
        store.openHud(session.id, spec: HudSpec(message: "one"), size: HudPanelSize(widthPercent: 20, heightPercent: 9))
        ...
        // an update repaints in place: same surface, same view identity.
    ...
    @Test func updateHudKeepsTheColorTheSurfaceWasCreatedWith() {
        ...
        let ws = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: ws.id, cwd: "/a")!
        store.openHud(session.id, spec: HudSpec(message: "one", backgroundColor: "#101820"),
    ...
    @Test func updateHudDropsAnOmittedTextColorWhileHoldingTheBackground() {
        ...
        let ws = store.addWorkspace(name: "work")
        let session = store.addSession(toWorkspace: ws.id, cwd: "/a")!
        store.openHud(session.id, spec: HudSpec(message: "one", backgroundColor: "#101820", textColor: "#e0e0e0"),
                      size: HudPanelSize(widthPercent: 20, heightPercent: 9))
    ...
    @Test func secondOpenHudReplacesTheFirst() {
        ...
        #expect(store.openHud(session.id, spec: next, size: HudPanelSize(widthPercent: 35, heightPercent: 9)) == true)
        #expect(session.hudSpec == next)
    ...
    @Test func openOverlayReplacesAHudButRefusesAProgram() {
        ...
        #expect(store.openOverlay(session.id, command: "other") == false)
        #expect(store.openHud(session.id, spec: HudSpec(message: "x"), size: HudPanelSize(widthPercent: 20, heightPercent: 9)) == false)
        #expect(session.overlayCommand == "htop")
        #expect(session.hudSpec == nil)
    }
    ...
    @Test func openHudUnknownSessionFails() {
        let store = makeStore()
        #expect(store.openHud(UUID(), spec: HudSpec(message: "x"), size: HudPanelSize(widthPercent: 20, heightPercent: 9)) == false)
