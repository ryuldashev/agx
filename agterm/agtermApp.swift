...
struct agtermApp: App {
    ...
    private static func restoredLibrary() -> WindowLibrary {
        ...
        library.sessionDiscardSink = { [weak library] session in
            ...
            // a private session's agent files go once its process has had a moment to exit (ADR 0003)
    ...
    /// exit-code file is minted per call, so two pane overlays open at once never share one. A HUD never
    /// reaches here: the deck renders it natively (`HudNoticeView`) and spawns no surface for it.
    @MainActor
    private static func makeOverlaySurface(for session: Session, store: AppStore, pane: OverlayPane?,
        ...
        let view = GhosttySurfaceView(workingDirectory: spec.cwd ?? session.effectiveCwd,
                                      ...
                                      waitAfterCommand: spec.wait, env: overlayEnv)
