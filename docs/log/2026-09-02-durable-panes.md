...
## Разбор жалобы: названия пропадали у сессий в неоткрытых воркспейсах
- Две причины, обе найдены по коду (без тестовых окон, по просьбе Руслана):
  1. Адоптация заголовка висела на `makeSurface` — а он срабатывает только при реализации панели
     (открытии воркспейса). Перенёс на restore: `WindowLibrary.adoptDurableTitles` подхватывает
     заголовок для сессий с живым abduco-сокетом сразу при загрузке стора.
  2. Заголовок терялся на ДЕКОДЕ с диска: у `SessionSnapshot` кастомный `init(from:)`, а `title`
     был добавлен только в `CodingKeys`+энкодер. Round-trip тесты в памяти это не ловили (идут мимо
     JSON). Новый `WindowLibraryTests.restoreAdoptsSavedTitleOnlyForASessionWithALiveDurableServer`
